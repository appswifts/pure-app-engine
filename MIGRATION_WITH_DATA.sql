-- Migration: Create restaurant_tables and migrate existing data
-- This will preserve any existing tables you already have
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tables') THEN
    ...
    RAISE NOTICE 'Migrated existing tables from old "tables" table to "restaurant_tables"';
  END IF;
END $$;

-- Optional: Show count of migrated tables
DO $$
DECLARE
  table_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO table_count FROM public.restaurant_tables;
  RAISE NOTICE 'Total tables in restaurant_tables: %', table_count;
END $$;
